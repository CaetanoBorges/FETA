...
dadosOperacao.para=para;this.modalConfirmar();}};class EnviarReq{constructor(jquery,apiUrl,loader,notificacao){this.jquery=jquery;this.apiUrl=apiUrl;this.loader=loader;this.notificacao=notificacao}
init(){var ver=db.verificaSessao();if(ver){db.verificaToken();return}
...
termosecondicoesWatchAct(){$("#termos").change(function(){if($(this).prop("checked")){localStorage.setItem("termos","1");clearInterval(window.termInterval);setTimeout(function(){history.back()},1000)}else{localStorage.setItem("termos","0")}});$("#privacidade").change(function(){if($(this).prop("checked")){localStorage.setItem("privacidade","1");clearInterval(window.privInterval);setTimeout(function(){history.back()},1000)}else{localStorage.setItem("privacidade","0")}})}
...
home(){var esse=this;var settings={"url":(this.apiUrl)+"/perfil/init","method":"GET","timeout":0,"headers":{"token":esse.db.getToken()},};$.ajax(settings).done(function(res){if(res.ok){var sessao=res.payload.bloqueio;if(sessao=="mins1"){localStorage.setItem("sessaolimite",60)}
...
verificaExistencia(){var esse=this;esse.loader.abrir();var settings={"url":(esse.apiUrl)+"/auth/verificaexistencia","method":"POST","timeout":0,"headers":{"Content-Type":"application/json"},"data":JSON.stringify({id:ESCOPO.dadosOperacao.id,bi:ESCOPO.dadosOperacao.bi}),};$.ajax(settings).done(function(response){if(!response.ok){esse.notificacao.sms("Código de confirmação enviado com sucesso",1);vaiTela("/inicioconfirmar")}else{esse.notificacao.sms(response.payload,1);esse.loader.fechar()}})}
confirmarTelefone(){var esse=this;esse.loader.abrir();var settings={"url":(esse.apiUrl)+"/auth/verificatelefone","method":"POST","timeout":0,"headers":{"Content-Type":"application/json"},"data":JSON.stringify({id:ESCOPO.dadosOperacao.id,codigo:$("#codigo").val()})};$.ajax(settings).done(function(response){if(response.ok){esse.notificacao.sms(response.payload,1);vaiTela("/criarpin")}else{esse.notificacao.sms(response.payload,1);esse.loader.fechar()}})}
...
ESCOPO.dadosOperacao.pin=pin_novo;var esse=this;esse.loader.abrir();var settings={"url":(this.apiUrl)+"/auth/cadastrar","method":"POST","timeout":0,"headers":{"Content-Type":"application/json"},"data":JSON.stringify(ESCOPO.dadosOperacao),};$.ajax(settings).done(function(response){if(response.ok){localStorage.setItem("sessao",Date.now());esse.db.setToken(response.token);esse.home();esse.db.verificaToken();esse.notificacao.sms("É feta, é fácil")}else{esse.notificacao.sms(response.payload,1);esse.loader.fechar()}})}};class PendentesReq{constructor(jquery,apiUrl,loader,notificacao){this.jquery=jquery;this.apiUrl=apiUrl;this.loader=loader;this.notificacao=notificacao}
...
ESCOPO.selectMes=new SlimSelect({select:'#mes',settings:{showSearch:!1,keepOrder:!0}});ESCOPO.selectMes.setData(mes);if(ESCOPO.init!=!0){$(".render-aqui").html(`<br><h4 style="text-align:center">SELECIONE O MÊS</h4><br>`);$("#qtd").html(" &nbsp; ")}});$('#mes').on("change",function(){var mes=String($('#mes').val());var ano=String($('#ano').val());if(mes!="00"&&mes!=0&&mes!="0"&&ESCOPO.init!=!0){esse.transacoes(mes,ano)}})}};class PagamentosReq{servicos={Recargas:["Unitel","Africel","Movicel","NetOne"],TV:["DSTV","ZAP","ZAP Fibra"],Estado:["ENDE","EPAL"],Outros:["Empresa X","Facebook","SimpleComerce"],}
